...
Context-Generic Programming (CGP) is a language extension for Rust, with pluggable trait implementations at compile-time. In ordinary Rust a trait has one implementation per type; CGP lets one trait have many interchangeable implementations and lets each _context_ choose which one it uses, so you can write functions and trait implementations that depend only on the _capabilities_ a context provides rather than on any single concrete type. It is an ordinary library on stable Rust that desugars to plain traits and impls, and the choice is resolved entirely during compilation, so the flexibility costs nothing at runtime.
